...
| `src/components/SmokeCursor.tsx` | The hero's ember light. A WebGL shader draws a faint, slowly drifting field of cloud (domain-warped noise), and the cursor acts as a light that illuminates whatever cloud is near it, so the glow takes the cloud's shape and a fast sweep leaves fading fragments rather than a streak. Falls back to a static CSS glow without WebGL. |
...
- **The cloud shader** renders at half resolution and stops entirely once the hero scrolls out of view. The cursor's light lives in a coarse CPU grid uploaded as a tiny texture each frame, so the per-frame cost is one small upload and one draw call.
- **The light fades on wall-clock time**, not per frame, so the trail behind the cursor lasts the same on a 30Hz laptop as on a 120Hz display.
- `prefers-reduced-motion` freezes the cloud light, and disables the dust and the scroll-expand pin.
